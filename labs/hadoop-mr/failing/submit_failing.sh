...
if [[ -z "$INPUT_PATH" ]];
then
  INPUT_PATH="/bdpc/hadoop_mr/failing/input"
fi

if [[ -z "$OUTPUT_PATH" ]];
then
  OUTPUT_PATH="/bdpc/hadoop_mr/failing/output"
fi
...
APP_PATH="$THIS_PATH/failing-1.0-jar-with-dependencies.jarr"
...
mapReduceArguments=(
  "$APP_PATH"
  "com.globallogic.bdpc.mapreduce.failing.Failing"
  "$INPUT_PATH"
  "$OUTPUT_PATH"
)
